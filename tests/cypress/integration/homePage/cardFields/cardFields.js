...
When(`I see right button is absent for card in Hired column`, () =>{
    return board.hiredColumn.getCard(1).rightButton.should("have.length", 0);
});

When(`I see left button is absent for card in Applied column`, () =>{
    return board.appliedColumn.getCard(1).leftButton.should("have.length", 0);
});

Then(`I see photo for card in Hired column`, () =>{
    return board.hiredColumn.getCard(1).photo.should("be.visible");
});

Then(`I see name for card in Hired column`, () =>{
    return board.hiredColumn.getCard(1).name.should("be.visible");
});

Then(`I see city name for card in Hired column`, () =>{
    return board.hiredColumn.getCard(1).city.should("be.visible");
});
